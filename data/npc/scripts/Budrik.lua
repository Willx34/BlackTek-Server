...
local function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, "yes") then
		if npcHandler.topic[cid] == 1 then
			...
			player:setStorageValue(PlayerStorageKeys.KillingInTheNameOf.BudrikMinos, 1)
			player:setStorageValue(PlayerStorageKeys.KillingInTheNameOf.BudrikMinosCount, 0)
			npcHandler.topic[cid] = 0
		...
		-- AQUI 
		if player:getStorageValue(PlayerStorageKeys.KillingInTheNameOf.BudrikMinos) <= 0 and player:getStorageValue(PlayerStorageKeys.toOutfoxAFoxQuest) == 2 then
			npcHandler:say({
		...
		elseif player:getStorageValue(PlayerStorageKeys.KillingInTheNameOf.BudrikMinos) == 1 then
			if player:getStorageValue(PlayerStorageKeys.KillingInTheNameOf.BudrikMinosCount) >= 5000 then
				npcHandler:say({
				...
				player:setStorageValue(PlayerStorageKeys.KillingInTheNameOf.BudrikMinos, 2)
			else
				npcHandler:say("Come back when you have slain {5000 minotaurs!}", cid)
			end
		elseif player:getStorageValue(PlayerStorageKeys.KillingInTheNameOf.BudrikMinos) == 2 then
			npcHandler:say({
				"It was very decent of you to help me, and I am thankful, really I am, but now I have to get back to my duties as a foreman."
			}, cid)
			player:setStorageValue(PlayerStorageKeys.KillingInTheNameOf.BudrikMinos, 3)
		elseif player:getStorageValue(PlayerStorageKeys.KillingInTheNameOf.BudrikMinos) == 3 then
