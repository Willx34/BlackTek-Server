...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "food") then
		...
		elseif player:getStorageValue(PlayerStorageKeys.Oramond.MissionToTakeRoots) == 1 then
			if player:getStorageValue(PlayerStorageKeys.Oramond.HarvestedRootCount) < 5 then
			...
			elseif player:getStorageValue(PlayerStorageKeys.Oramond.HarvestedRootCount) >= 5 then
				npcHandler:say({
				...
				player:setStorageValue(PlayerStorageKeys.Oramond.MissionToTakeRoots, 0)
			end
		end
